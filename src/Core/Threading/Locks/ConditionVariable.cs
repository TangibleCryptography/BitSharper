...
namespace BitSharper.Threading.Locks
{
    ...
    internal class ConditionVariable : ICondition // BACKPORT_3_1
    {
        ...
        protected internal readonly IExclusiveLock _lock;
        ...
        public virtual void AwaitUninterruptibly()
        {
            ...
            try
            {
                lock (this)
                {
                    for (var i = holdCount; i > 0; i--) _lock.Unlock();
                    try
                    {
                        Monitor.Wait(this);
                    }
            ...
            {
                for (var i = holdCount; i > 0; i--) _lock.Lock();
                if (wasInterrupted)
                {
