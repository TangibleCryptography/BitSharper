...
{
    /// <summary> 
    /// Provides default implementations of <see cref="BitSharper.Threading.Execution.IExecutorService"/>
    /// execution methods. 
    /// </summary>
    /// <remarks> 
    /// This class implements the <see cref="BitSharper.Threading.Execution.IExecutorService"/> methods using a
    ...
    /// <p/>
    /// For example, the implementation of <see cref="BitSharper.Threading.Execution.IExecutorService.Submit(IRunnable)"/> creates an
    ...
    /// that customizes <see cref="BitSharper.Threading.Execution.ThreadPoolExecutor"/> to use
    ...
    internal abstract class AbstractExecutorService : IExecutorService //JDK_1_6
    {
        ...
        /// either <see cref="BitSharper.Threading.Execution.IExecutorService.Shutdown()"/> or 
        /// <see cref="BitSharper.Threading.Execution.IExecutorService.ShutdownNow()"/> was called first.
        /// </remarks>
        /// <returns> <c>true</c> if all tasks have completed following shut down
        /// </returns>
        ...
        /// thread, at the discretion of the <see cref="BitSharper.Threading.IExecutor"/> implementation.
        /// </remarks>
        ...
        {
            var collection = tasks as ICollection<ICallable<T>>;
            var count = collection == null ? 0 : collection.Count;
        ...
        {
            var collection = tasks as ICollection<Func<T>>;
            var count = collection == null ? 0 : collection.Count;
        ...
        private static T SetContextCarrier<T>(T task, IContextCarrier contextCarrier)
        {
        ...
        private T DoInvokeAny<T>(IEnumerable tasks, int count, bool timed, TimeSpan durationToWait, Converter<object, IRunnableFuture<T>> converter)
        {
            ...
            var futures = count > 0 ? new List<IFuture<T>>(count) : new List<IFuture<T>>();
            var ecs = new ExecutorCompletionService<T>(this);
        ...
        private List<IFuture<T>> DoInvokeAll<T>(IEnumerable tasks, int count, Converter<object, IRunnableFuture<T>> converter)
        {
            ...
            try
            {
                ...
                {
                    var runnableFuture = SetContextCarrier(converter(task), contextCarrier);
